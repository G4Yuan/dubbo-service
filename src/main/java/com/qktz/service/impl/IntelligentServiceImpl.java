...
import com.qktz.model.AmoySharePool;
...
import com.qktz.model.MasterStock;
import com.qktz.model.MasterStockPool;
import com.qktz.model.MasterStockPoolOperatorFlow;
import com.qktz.model.MasterStockPoolTmp;
...
public class IntelligentServiceImpl implements IntelligentService.Iface {
	 ...
	 */
	@Override
	public int saveAndUpdateAmoySharePool(String reqId, long userId, AmoySharePoolInfo info)
			throws BusinessException, TException {
		long startTime = System.currentTimeMillis();
		logger.info("#{}#{}#userId={},info={}#", reqId, "保存或者更新公告淘股池数据", userId, new Gson().toJson(info));
		if (info == null || info.getStock_notice() == null || info.getStock_notice().size() <= 0
				|| info.getStock_today() == null || info.getStock_today().size() <= 0)
			throw new BusinessException(99998, "非法参数");
		int id = info.getId();
		boolean isNew = true;
		if (id > 0)
			isNew = false;
		AmoySharePool asp = null;
		if (isNew) {
			asp = new AmoySharePool();
			asp.setCreator(userId);
			asp.setCreateTime(new Date());
		} else {
			asp = AmoySharePool.dao.findById(id);
			asp.setModifier(userId);
			asp.setModifyTime(new Date());
		}
		if (info.getStock_today() != null && info.getStock_today().size() > 0)
			asp.setStockToday(JSON.toJSONString(info.getStock_today()));
		if (info.getStock_notice() != null && info.getStock_notice().size() > 0)
			asp.setStockNotice(JSON.toJSONString(info.getStock_notice()));
		if (isNew) {
			asp.save();
		} else {
			asp.update();
		...
		try {
			ActiveMqTools.sendTopicMsg(Constants.TOPIC_BOSS_AMOY_STOCK_POOl, JSON.toJSONString(asp.copy()));
		} catch (Exception e) {
			logger.error("", e);
		}
		long endTime = System.currentTimeMillis();
		logger.info("#{}#接口耗时{}ms#", reqId, (endTime - startTime));
		return Integer.parseInt(asp.getId() + "");
	}
	 ...
	 */
	@Override
	public AmoyPager findByPager(String reqId, int pageNow, int pageSize) throws BusinessException, TException {
		logger.info("#{}#{}#参数为:pageNow={},pageSize={}#---", reqId, "分页查找公告淘股数据", pageNow, pageSize);
		long startTime = System.currentTimeMillis();
		if (pageSize != 0 && pageNow <= 0)
			return new AmoyPager();
		AmoyPager pager = new AmoyPager();
		int pageNum = 0;
		Long pageRecords = Db.queryLong("select count(*) count from amoy_share_pool");
		if (pageSize != 0) {
			// 分页查询
			int recordSize = Integer.parseInt(pageRecords + "");
			pageNum = recordSize > 0 ? (recordSize % pageSize == 0 ? recordSize / pageSize : recordSize / pageSize + 1)
			...
			pager.setRecordSize(Integer.parseInt(pageRecords + ""));
			pager.setPageSize(pageSize);
			pager.setRecords(AmoySharePool.dao.findByPager(offset, pageSize));
			...
			pager.setRecordSize(Integer.parseInt(pageRecords + ""));
			pager.setPageSize(pageSize);
			pager.setRecords(AmoySharePool.dao.findByPager(0, 0));
		}
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "分页查找公告淘股数据", (endTime - startTime));
		return pager;
	}
	 ...
	 */
	@Override
	public AmoySharePoolInfo findCurrentAmoySharePool(String reqId) throws BusinessException, TException {
		logger.info("#{}#{}#---", reqId, "查找公告淘股最新数据");
		long startTime = System.currentTimeMillis();
		AmoySharePoolInfo info = new AmoySharePoolInfo();
		info = AmoySharePool.dao.findCurrentRecord();
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "查找公告淘股最新数据", (endTime - startTime));
		return info;
	}
	 ...
	 */
	@Override
	public AmoySharePoolInfo findAmoySharePoolById(String reqId, long recordId) throws BusinessException, TException {
		logger.info("#{}#{}#---", reqId, "查找公告淘股最新数据");
		long startTime = System.currentTimeMillis();
		AmoySharePoolInfo info = new AmoySharePoolInfo();
		AmoySharePool asp = AmoySharePool.dao.findById(recordId);
		if (asp != null) {
			info = asp.copy();
		}
		long endTime = System.currentTimeMillis();
	 ...
	 */
	@Override
	public synchronized int saveAndEditMasterStockPool(String reqId, long userId, BaseMasterStockPool info)
			throws BusinessException, TException {
		logger.info("#{}#{}#userId={}#info={}#---", reqId, "保存或者编辑高手股票池", userId, JSON.toJSONString(info));
		long startTime = System.currentTimeMillis();
		/**
		 * 参数校验
		 */
		if (info == null || info.getStocks() == null || info.getStocks().size() <= 0)
			throw new BusinessException(99998, "非法参数");
		long stockPoolId = info.getId();
		boolean isNew = true;
		if (stockPoolId > 0)
			isNew = false;
		try {
			MasterStockPool stockPool = null;
			if (isNew) {
				stockPool = new MasterStockPool();
				stockPool.setName(info.getName());
				stockPool.setStrategyRemark(info.getStrategy_remark());
				stockPool.setLabels(StringUtils.join(info.getLabels(), ","));
				stockPool.setStatus(MasterStockPoolStatus.PENDING_REVIEW.getCode());
				stockPool.setSummary(info.getSummary());
				stockPool.setCreator(userId);
				stockPool.setCreateTime(new Date());
				stockPool.setModifier(userId);
				stockPool.setModifyTime(new Date());
				stockPool.save();
				stockPoolId = stockPool.getId();
				/**
				 * 缓存表中保存一条缓存数据
				 */
				MasterStockPoolTmp stockPoolTemp = new MasterStockPoolTmp();
				stockPoolTemp.copyToTemp(stockPool);
				
			} else {
				// 每次进来清理掉上一次的数据
				Db.update("delete from master_stock where master_id=? and (status=2 or status=4)", new Object[]{stockPoolId});
				/**
				 * 查找该股票池数据
				 */
				stockPool = MasterStockPool.dao.findById(stockPoolId);
				MasterStockPoolTmp stockPoolTemp = null;
				// 如果是发布状态修改
				if(stockPool.getStatus() == MasterStockPoolStatus.OK.getCode()){
					stockPoolTemp = MasterStockPoolTmp.dao.findById(stockPoolId);
					if(stockPoolTemp == null){
						stockPoolTemp = new MasterStockPoolTmp();
						/**
						 * 缓存表保存一份数据
						 */
						stockPoolTemp.copyToTemp(stockPool);
				...
				stockPoolTemp.setId(stockPoolId);
				if(StringUtils.isNotBlank(info.getName()))
					stockPoolTemp.setName(info.getName());
				if(info.getLabels()!=null && info.getLabels().size()>0)
					stockPoolTemp.setLabels(StringUtils.join(info.getLabels(), ","));
				if(StringUtils.isNotBlank(info.getSummary()))
					stockPoolTemp.setSummary(info.getSummary());
				if(StringUtils.isNotBlank(info.getStrategy_remark()))
					stockPoolTemp.setStrategyRemark(info.getStrategy_remark());
				stockPoolTemp.setStatus(MasterStockPoolStatus.PENDING_REVIEW.getCode());
				stockPoolTemp.setModifier(userId);
				stockPoolTemp.setModifyTime(new Date());
				stockPoolTemp.update();
				
				
				List<MasterStock> out = new ArrayList<MasterStock>();	// 调出
				/**
				 * 股票池数据处理
				 */
				List<MasterStockInfo> stocks = info.getStocks();
				logger.info("#{}#高手股票池:传入{}#", reqId, new Gson().toJson(stocks));
				// 本次调入
				Set<String> inStocks = new HashSet<String>();
				StringBuffer strBuffer = new StringBuffer();
				for(MasterStockInfo item:stocks){
					strBuffer.append("'").append(item.getStock_code()).append("',");
					inStocks.add(item.getStock_code());
				}
				strBuffer.deleteCharAt(strBuffer.length()-1);
				logger.info("#{}#{}#",reqId, strBuffer.toString());
				out = MasterStock.dao.findByOut(stockPoolId, strBuffer.toString());
				// 保存一模一样的
				for(MasterStock item:out){
					// 如果已经调出的话
//					MasterStock tmp = MasterStock.dao.findByPoolCodeAndStatus(stockPoolId, item.getStockCode(), 3);
//					if(tmp != null){
//						tmp.delete();
//					}
					item.setId(null);
				...
				logger.info("#{}#高手股票池:调出{}#", reqId, new Gson().toJson(out));
				Db.batchSave(out, out.size());
			}
			/**
			 * 新增股票池统一为调入用状态
			 */
			List<MasterStockInfo> stocks = info.getStocks();
			List<MasterStock> ss = new ArrayList<MasterStock>();
			MasterStock stockInfo = null;
			for (MasterStockInfo item : stocks) {
				stockInfo = new MasterStock();
				String stockCode = item.getStock_code();
				if(StringUtils.isBlank(stockCode)) continue;
				MasterStock tmp = MasterStock.dao.findByPoolCodeAndStatus(stockPoolId, stockCode, 1);
				if(tmp != null){
					stockInfo.setModifier(tmp.getModifier());
					stockInfo.setModifyTime(tmp.getModifyTime());
					stockInfo.setCreateTime(tmp.getCreateTime());
					stockInfo.setCreator(tmp.getCreator());
				}else{
					stockInfo.setCreator(userId);
					stockInfo.setCreateTime(new Date());
					stockInfo.setModifier(userId);
					stockInfo.setModifyTime(new Date());
				}
				stockInfo.copy(item);
				stockInfo.setStatus(2);
				stockInfo.setMasterId(stockPoolId);
				ss.add(stockInfo);
			}
			Db.batchSave(ss, ss.size());
			/**
			 * 保存数据
			 */
			MasterStockPoolOperatorFlow flow = new MasterStockPoolOperatorFlow();
			flow.setMasterId(stockPoolId);
			flow.setOperation(MasterStockPoolStatus.PENDING_REVIEW.getMsg());
			flow.setOperatorName("");// 如果需要则跑数据即可
			flow.setOperator(userId);
			flow.setOperatorTime(new Date());
			flow.save();
		} catch (Exception e) {
			logger.error("", e);
		}
		...
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "保存或者编辑高手股票池", (endTime - startTime));
		return Integer.parseInt(stockPoolId + "");
	}
	 ...
	 */
	@Override
	public synchronized int operatorMasterStockPool(String reqId, long userId, long masterId, int status, String remark)
			throws BusinessException, TException {
		logger.info("#{}#{}#userId={}, masterId={},status={},remark={}#---", reqId, "操作高手股票池状态", userId, masterId, status, remark);
		long startTime = System.currentTimeMillis();
		try {
			MasterStockPool stockPool = MasterStockPool.dao.findById(masterId);
			if (stockPool != null) {
				if(MasterStockPoolStatus.OK.getCode() == status){
					/**
					 * 消息
					 */
					...
					MasterStockPoolTmp tmp = MasterStockPoolTmp.dao.findById(masterId);
					if(tmp != null){
						stockPool.setName(tmp.getName());
						stockPool.setLabels(tmp.getLabels());
						stockPool.setStrategyRemark(tmp.getStrategyRemark());
						stockPool.setSummary(tmp.getSummary());
						stockPool.setStatus(status);
						stockPool.setModifier(userId);
						stockPool.setModifyTime(new Date());
						stockPool.update();
						List<MasterStock> instocks = 
								MasterStock.dao.find(Constants.MASTER_STOCK_OUT, 
										new Object[]{masterId, masterId, masterId});
						JSONObject inObj = null;
						for(MasterStock item:instocks){
							inObj = new JSONObject();
							inObj.put("stock_code", item.getStockCode());
							inObj.put("stock_name", item.getStockName());
							inObj.put("current_price", item.getCurrentPrice());
							in.add(inObj);
						}
						/**
						 * 清理1状态的
						 */
						Db.update(Constants.MASTER_STOCK_DELTE, 
								new Object[]{masterId, 1});
						/**
						 * 更新调出股票为正常状态
						 */
						List<MasterStock> ostocks = 
								MasterStock.dao.find(Constants.MASTER_STOCK_QUERY, 
										new Object[]{masterId, 4});
						JSONObject outObj = null;
						for(MasterStock item:ostocks){
							item.setStatus(3);
							...
							outObj.put("stock_code", item.getStockCode());
							outObj.put("stock_name", item.getStockName());
							outObj.put("current_price", item.getCurrentPrice());
							out.add(outObj);
						}
						Db.batchUpdate(ostocks, ostocks.size());
						/**
						 * 更新调入股票为正常状态
						 */
						List<MasterStock> mstocks = 
								MasterStock.dao.find(Constants.MASTER_STOCK_QUERY, 
										new Object[]{masterId, 2});
						
						for(MasterStock item:mstocks){
							item.setStatus(1);
							for(Object obj:in){
								JSONObject o = (JSONObject) obj;
								if(o != null){
									String scode = o.getString("stock_code");
									if(scode != null){
										if(scode.equals(item.getStockCode())){
											item.setModifier(userId);
											item.setModifyTime(new Date());
										}
									}
						...
						Db.batchUpdate(mstocks, mstocks.size());
						tmp.delete();
					}
					
					mqMsg.put("in", in);
					mqMsg.put("out", out);
					ActiveMqTools.sendTopicMsg(Constants.TOPIC_QTG_MASTER_SHARE_V2, mqMsg.toJSONString());
				}else{
					// 其他做状态处理
					stockPool.setStatus(status);
					stockPool.setModifier(userId);
				...
				flow.setOperation(MasterStockPoolStatus.getMsg(status));
				flow.save();
			}
		} catch (Exception e) {
			logger.error("", e);
		...
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "操作高手股票池状态", (endTime - startTime));
		return 1;
	}
	 ...
	 */
	@Override
	public MasterStockPoolPager findMasterStockPoolPager(String reqId, MasterQueryCondition condition, int pageNow, int pageSize)
			throws BusinessException, TException {
		logger.info("#{}#{}#pageNow={},pageSize={}#---", reqId, "分页获取高手股票池数据", pageNow, pageSize);
		long startTime = System.currentTimeMillis();
		if(pageSize != 0 && pageNow <=0) return new MasterStockPoolPager();
		MasterStockPoolPager pager = new MasterStockPoolPager();
		int pageNum = 0;
		StringBuffer strBuff = new StringBuffer(Constants.MASTER_STOCK_UNION_CNT);
		if(condition != null){
			if(StringUtils.isNotBlank(condition.getName()))
				strBuff.append(" and d.name like '%"+condition.getName()+"%'");
			if(condition.getStatus() >0 )
				strBuff.append(" and d.status="+condition.getStatus());
		}
		try {
			Long pageRecords = Db.queryLong(strBuff.toString());
			if(pageSize != 0){
				// 分页查询
				int recordSize = Integer.parseInt(pageRecords+"");
				pageNum = recordSize>0?(recordSize%pageSize==0?recordSize/pageSize:recordSize/pageSize+1):0;
				int offset = (pageNow-1)*pageSize;
				...
				pager.setRecordSize(Integer.parseInt(pageRecords+""));
				pager.setPageSize(pageSize);
				pager.setRecords(MasterStockPool.dao.findByPager(condition, offset, pageSize));
				...
				pager.setRecordSize(Integer.parseInt(pageRecords+""));
				pager.setPageSize(pageSize);
				pager.setRecords(MasterStockPool.dao.findByPager(condition, 0, 0));
			}
		} catch (Exception e) {
			logger.error("", e);
		}
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "分页获取高手股票池数据", (endTime - startTime));
		return pager;
	}
	 ...
	 */
	@Override
	public BaseMasterStockPool findMasterStockPoolDetail(String reqId, long stockPoolId)
			throws BusinessException, TException {
		logger.info("#{}#{}#---", reqId, "获取股票池详情");
		long startTime = System.currentTimeMillis();
		BaseMasterStockPool detail = new BaseMasterStockPool();
		try {
			MasterStockPool pool = MasterStockPool.dao.findById(stockPoolId);
			if (pool != null) {
				detail.setId(pool.getId());
				detail.setName(pool.getName());
				detail.setStatus(pool.getStatus());
				detail.setSummary(pool.getSummary());
				detail.setStrategy_remark(pool.getStrategyRemark());
				String label = pool.getLabels() == null ? "" : pool.getLabels();
				detail.setLabels(Arrays.asList(label.split(",")));
				detail.setStocks(MasterStock.dao.findMasterStockInfoList(pool.getId(), 1));
				detail.setOperations(MasterStockPoolOperatorFlow.dao.findListByPoolId(pool.getId()));
				detail.setCreator(pool.getCreator());
				detail.setCreate_time(new SimpleDateFormat(Constants.DETAUL_DATE_FORMAT).format(pool.getCreateTime()));
				detail.setModifier(pool.getModifier());
				detail.setHistory_stocks(MasterStock.dao.findMasterStockInfoList(pool.getId(),3));
				detail.setModify_time(new SimpleDateFormat(Constants.DETAUL_DATE_FORMAT).format(pool.getModifyTime()));
				detail.setOper_timestamp(pool.getModifyTime().getTime());
				/**
				 * 获取领涨股
				 */
				MasterStock stock = MasterStock.dao.findFirst(Constants.MASTER_STOCK, new Object[] { pool.getId(), 1 });
				if (stock != null){
					detail.setStock_code(stock.getStockCode());
					detail.setStock_name(stock.getStockName());
					detail.setTotal_yield(stock.getCurrentYield());
				}
				long count = Db.queryLong(Constants.MASTER_STOCK_CNT, new Object[] {pool.getId(), 1 });
		...
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "获取股票池详情", (endTime - startTime));
		return detail;
	}
	/**
	 * boss端获取股票池详情
	 ...
	 */
	@Override
	public BaseMasterStockPool findStockPoolBossDetail(String reqId, long stockPoolId)
			throws BusinessException, TException {
		logger.info("#{}#{}#---", reqId, "获取boss股票池详情");
		long startTime = System.currentTimeMillis();
		BaseMasterStockPool detail = new BaseMasterStockPool();
		try {
			MasterStockPoolTmp pool = MasterStockPoolTmp.dao.findById(stockPoolId);
			if(pool != null) {
				detail.setId(pool.getId());
				detail.setName(pool.getName());
				detail.setStatus(pool.getStatus());
				...
				String label = pool.getLabels() == null ? "" : pool.getLabels();
				detail.setLabels(Arrays.asList(label.split(",")));
				detail.setStocks(MasterStock.dao.findMasterStockInfoList(pool.getId(), 2));
				detail.setOperations(MasterStockPoolOperatorFlow.dao.findListByPoolId(pool.getId()));
				detail.setCreator(pool.getCreator());
				detail.setCreate_time(new SimpleDateFormat(Constants.DETAUL_DATE_FORMAT).format(pool.getCreateTime()));
				detail.setModifier(pool.getModifier());
				detail.setHistory_stocks(MasterStock.dao.findMasterStockInfoList(pool.getId(),4));
				detail.setModify_time(new SimpleDateFormat(Constants.DETAUL_DATE_FORMAT).format(pool.getModifyTime()));
				detail.setOper_timestamp(pool.getModifyTime().getTime());
				/**
				 * 获取领涨股
				 */
				MasterStock stock = MasterStock.dao.findFirst(Constants.MASTER_STOCK, new Object[] { pool.getId(), 2 });
				if (stock != null){
					detail.setStock_code(stock.getStockCode());
					detail.setStock_name(stock.getStockName());
					detail.setTotal_yield(stock.getCurrentYield());
				}
				long count = Db.queryLong(Constants.MASTER_STOCK_CNT, new Object[] { stockPoolId, 2 });
				detail.setCount(count);	
			}else{
				MasterStockPool pool_t = MasterStockPool.dao.findById(stockPoolId);
				detail.setId(pool_t.getId());
				detail.setName(pool_t.getName());
				detail.setStatus(pool_t.getStatus());
				detail.setSummary(pool_t.getSummary());
				detail.setStrategy_remark(pool_t.getStrategyRemark());
				String label = pool_t.getLabels() == null ? "" : pool_t.getLabels();
				detail.setLabels(Arrays.asList(label.split(",")));
				detail.setStocks(MasterStock.dao.findMasterStockInfoList(pool_t.getId(), 1));
				detail.setOperations(MasterStockPoolOperatorFlow.dao.findListByPoolId(pool_t.getId()));
				detail.setCreator(pool_t.getCreator());
				detail.setCreate_time(new SimpleDateFormat(Constants.DETAUL_DATE_FORMAT).format(pool_t.getCreateTime()));
				detail.setModifier(pool_t.getModifier());
				detail.setHistory_stocks(MasterStock.dao.findMasterStockInfoList(pool_t.getId(),3));
				detail.setModify_time(new SimpleDateFormat(Constants.DETAUL_DATE_FORMAT).format(pool_t.getModifyTime()));
				detail.setOper_timestamp(pool_t.getModifyTime().getTime());
				/**
				 * 获取领涨股
				 */
				MasterStock stock = MasterStock.dao.findFirst(Constants.MASTER_STOCK, new Object[] { pool_t.getId(), 1 });
				if (stock != null){
					detail.setStock_code(stock.getStockCode());
					detail.setStock_name(stock.getStockName());
					detail.setTotal_yield(stock.getCurrentYield());
				}
				long count = Db.queryLong(Constants.MASTER_STOCK_CNT, new Object[] {pool_t.getId(),  1 });
				detail.setCount(count);	
			}
		} catch (Exception e) {
			logger.error("", e);
		...
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "获取boss股票池详情", (endTime - startTime));
		return detail;
	}
	/**
	 * 获取股票池列表
	 ...
	 */
	@Override
	public List<StockShortInfo> findStockPoolList(String reqId) throws BusinessException, TException {
		logger.info("#{}#{}#---", reqId, "获取股票池列表");
		long startTime = System.currentTimeMillis();
		List<StockShortInfo> stocks = new ArrayList<StockShortInfo>();
		StockShortInfo stockInfo = null;
		try {
			List<MasterStockPool> msps = MasterStockPool.dao.find(Constants.MASTER_STOCK_POOL);
			if (msps != null && msps.size() > 0) {
				for (MasterStockPool item : msps) {
					MasterStock stock = MasterStock.dao.findFirst(Constants.MASTER_STOCK, new Object[] { item.getId(), 1 });
					if (stock == null)
						continue;
					long count = Db.queryLong(Constants.MASTER_STOCK_CNT, new Object[] { item.getId(), 1 });
					stockInfo = new StockShortInfo();
					stockInfo.setId(item.getId());
					stockInfo.setStock_code(stock.getStockCode());
					stockInfo.setStock_name(stock.getStockName());
					stockInfo.setStock_pool_name(item.getName());
					stockInfo.setStrategy_remark(item.getStrategyRemark());
					stockInfo.setStrategy(stock.getSubject());
					stockInfo.setTotal_yield(stock.getCurrentYield());
					stockInfo.setCount(Integer.parseInt(count + ""));
					String label = item.getLabels() == null ? "" : item.getLabels();
					stockInfo.setLabels(Arrays.asList(label.split(",")));
					if(item.getModifyTime() != null){
						stockInfo.setLeast_modify_time(new SimpleDateFormat(Constants.DETAUL_DATE_FORMAT).format(item.getModifyTime()));
						stockInfo.setOper_timestamp(item.getModifyTime().getTime());
					}
		...
		long endTime = System.currentTimeMillis();
		logger.info("#{}#{}耗时:{}ms---", reqId, "获取股票池列表", (endTime - startTime));
		return stocks;
	}
	  /**
		* @param reqId
